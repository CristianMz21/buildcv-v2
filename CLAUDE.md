...
- **Logout authenticates against a second JWT scheme.** `AuthenticationSchemes.ExpiredAccessTokenAllowed` is identical to the default scheme except `ValidateLifetime = false`, and `/auth/logout` reaches it through an explicit `AuthenticateAsync` call. No authorization policy names it and it is not the default, so an expired token still opens nothing else. This exists because the usual logout is "idle tab, token already expired": validating lifetime there would reduce logout to clearing cookies and revoking nothing, which is the bug the endpoint exists to close. For the same reason the **access-token cookie now expires with the session (the refresh token), not with the JWT** — the browser used to delete the only credential naming the account at the same instant the token went stale. The JWT's `exp` is still the security control everywhere else.
- `/auth/logout` is `AllowAnonymous` so a caller with no usable credential can still clear its cookies, and carries its own `RateLimitPolicies.Logout` window (20/min per IP) because authentication used to be its de facto throttle. It is still CSRF-guarded and must stay out of `CsrfGuardMiddleware.ExemptPaths`. A cookie-based client must re-fetch `GET /auth/antiforgery` before logging out if its access token has expired — the antiforgery token is bound to the principal, so an authenticated-bound token fails validation once the caller reads as anonymous.
