...
- **`importEvidence` is the one field of the body that is NOT part of the draft.** `POST /resumes/import/propose` mints an HMAC-signed opaque token — `<payload>.<signature>`, base64url, a fixed 35-byte payload bound to the `AccountId` with a 2-hour lifetime — and the client posts it back inside the draft object it received. `ResumeDraft` must not grow a field for it: that type is untrusted text the candidate corrects, and this is a credential a hand-edit invalidates, so it travels on `CreateResumeFromDraftCommand` instead and enters the aggregate through the same single `Resume.Create`. A token that fails verification is a **field error at `importEvidence`**, collected beside the draft's own errors rather than short-circuiting them, and creates nothing. **`/import/propose` still writes nothing** — that is the whole reason for a token rather than a persisted draft row: there are no background jobs, so a row written at extract time would have no garbage collector for abandoned imports.
- **Signing reuses the blind-index ring** (`Infrastructure/Security/ImportEvidenceProtector`, context `"Resume.ImportEvidence"`), which buys the length-prefixed-context framing, one place that touches HMAC keys, and the write-active-key / read-**all**-keys rotation rule a signature needs — a token minted the moment before a key roll must still verify after it. **The order of the checks is the security property**: nothing is read out of the payload until the signature verifies, then the account, then the expiry. The protector is deliberately NOT a dependency of `ProposeResumeDraftFromDocumentHandler` — that handler depends on exactly the two read-only extraction ports and a test reads its constructor to keep it so; the composition root signs.
...
- **`AtsParseability` applies exactly when the resume carries `ImportSignals`, and the score and the applicability move together.** `ReadabilityEngine` reads `resume.ImportSignals` once and both branches key off that one value; a resume typed by hand renormalizes the section out and still reaches 1.00, and a cleanly exported single-column PDF scores the section in full and *also* reaches 1.00. Applicability without a real score would give the section its 0.10 weight against a hard zero and cap every importer at 0.90 — the failure `ScoringWeightsSnapshot.RenormalizedTo`'s remark on Languages describes — so `AtsParseabilityTests.A_perfect_single_column_pdf_with_a_text_layer_scores_one_hundred_not_ninety` asserts the weight *and* the total, because renormalizing the section straight back out would also total 1.00.
- **The section is a share of what could be measured, and `Unknown` leaves the denominator.** Two terms: the document yielded machine-readable text without OCR, and it reads in one column. A non-PDF carries no geometry, so the column term is dropped from both halves of the fraction rather than scored as a failure — penalising `Unknown` would charge a DOCX, the most ATS-parseable format there is, for the detector's silence. `PageCount` is persisted and deliberately unscored: an ATS parses a long PDF as well as a short one, and how much to write is Completeness's and Achievements' business.
- **Its advice is the only advice that names an edit to a *file*, and every sentence ends in "import it again".** The document is never stored, so re-exporting alone cannot change a resume's stored signals — acting on it produces a second resume. Two accepted limits, stated on `ImportSignals` and in the OpenAPI description: the signals describe the document that was uploaded rather than the CV as it now stands, and the token is bound to an **account**, so a candidate may import a different CV of their own with it. Both are inherent to never keeping the file.
...
`Resume.ImportSignals` is the case that *feels* like candidate data and is not: an optional owned value in four nullable `Import_*` columns on the Resumes row, holding two closed enums, a bool and a page count. It describes a **PDF**, not a person, so nothing in it can carry a fragment of the document — which is exactly why the free-text extraction warnings never reach it and `ImportWarningFlags` is a closed `[Flags]` enum instead (one of those strings quotes a heading the candidate wrote). All four are read by the readability engine on every run and are what "how many candidates upload two-column PDFs" is answered from, so encrypting any of them would end the only thing they exist for.
